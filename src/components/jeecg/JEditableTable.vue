      ...
      <div class="scroll-view" ref="scrollView" :style="{'max-height':maxHeight+'px'}">
        ...
        <div :id="`${caseId}tbody`" class="tbody" :style="tbodyStyle">
          ...
          >
            ...
            <div v-if="rowNumber" class="td td-num" :style="style.tdLeft">
              <span v-if="!rowSelection">合计</span>
            </div>
  ...
  export default {
    ...
    methods: {
      ...
      _pushByDataSource(dataSource, insertIndexes = null, update = true, rows = null, setDefaultValue = false) {
        ...
        let popupGoodsValues = { ...this.popupGoodsValues }
        ...
        dataSource.forEach((data, newValueIndex) => {
          ...
          this.columns.forEach(column => {
            ...
            } else if (column.type === FormTypes.popupGoods) {
              popupGoodsValues[inputId] = sourceValue
        ...
        this.popupGoodsValues = popupGoodsValues
        this.radioValues = radioValues
        this.multiSelectValues = multiSelectValues
        console.log("_pushByDataSource multiSelectValues=",this.multiSelectValues)
        ...
        if (update) {
          this.rows = rows
          console.log("_pushByDataSource this.rows",this.rows)
          console.log("_pushByDataSource rows",rows)
          // 更新form表单的值
          this.$nextTick(() => {
            console.log("_pushByDataSource 调用 forceUpdateFormValues")
        ...
        for (let value of inputValues) {
          ...
          this.columns.forEach(column => {
            ...
            if (column.type === FormTypes.checkbox) {
              ...
              if (!value[column.key]) {
                console.log("getValuesAsync value[column.key]=",this.popupGoodsValues[inputId] || null)
                value[column.key] = this.popupGoodsValues[inputId] || null
              }
            } else if (column.type === FormTypes.radio) {
      ...
      _getAllValuesForSlot() {
        return cloneObject({
          ...
          popupGoodsValues: this.popupGoodsValues,
      ...
      /** 设置某行某列的值 */
      setValues(values) {
        console.log("enter setValues",values)
        values.forEach(item => {
          let { rowKey, values: newValues } = item
          rowKey = this.getCleanId(rowKey)
          console.log("setValues rowKey",rowKey)
          for (let newValueKey in newValues) {
            console.log("setValues newValueKey",newValueKey)
                ...
                if (column.key === newValueKey) {
                  ...
                  if (!edited) {
                    ...
                    if (column.type === FormTypes.select) {
                      ...
                      console.log("setValues this.popupGoodsValues",newValue)
                      edited = this.setOneValue(this.popupGoodsValues, modelKey, newValue)
                    } else if (column.type === FormTypes.radio) {
      ...
      handleChangePopupGoodsCommon(value, id, row, column,index) {
        console.log("handleChangePopupGoodsCommon",value,id,row,column,index)
        this.popupGoodsValues = this.bindValuesChange(value, id, 'popupGoodsValues')
        // 做单个表单验证
        this.validateOneInput(value, row, column, this.notPassedIds, true, 'change')
      ...
      bindValuesChange(value, id, key) {
        console.log("bindValuesChange this[key],id,value",this[key], id, value)
      ...
      popupCallback(value, others, id, row, column, index) {
       ...
       /** popupGoods回调 */
      popupGoodsCallback(value, others, id, row, column, index) {
        // 存储输入的值
        console.log("popupGoodsCallback");
        this.popupGoodsValues[id] = value
        if (others) {
            ...
            this.columns.map(k=>{
              if(k.key === key){
                ...
                if(k.type === 'date'){
                  ...
                  this.popupGoodsValues[index][key] = others[key]
                }
              }
            })
          })
        ...
        // 更新form表单的值
        this.$nextTick(() => {
          console.log("popupGoodsCallback 调用forceUpdateFormValues")
          this.forceUpdateFormValues()
        })
      },
      /** popup输入框回显 */
      ...
      /** popupGoods输入框回显 */
      getPopupGoodsValue(id) {
        console.log("getPopupGoodsValue[id],id",this.popupGoodsValues[id],id);
        return this.popupGoodsValues[id]
      },
      /** popupJsh构造传值 */
      getPopupGoodsRows(row) {
        console.log("getPopupGoodsRows");
        let { values } = this.getValuesSync({ validate: false, rowIds: [row.id] })
