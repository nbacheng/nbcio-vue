  ...
  <div class="container">
    ...
    <div class="center-board">
      <div class="action-bar">
        ...
        <!-- cancel by nbacheng <el-button icon="el-icon-video-play" type="text" @click="run">
          运行
        </el-button> -->
...
export default {
  ...
  mounted() {
    ...
    if (id) {
      getForm(id).then(res =>{
        that.formConf = JSON.parse(res.result.formContent);
  ...
  methods: {
    ...
    activeFormItem(currentItem) {
      ...
      this.activeId = currentItem.__config__.id
    },
    onEnd(obj) {
      if (obj.from !== obj.to) {
    ...
    updateDrawingList(newTag, list) {
      const index = list.findIndex(item => item.__config__.id === this.activeId)
      if (index > -1) {
